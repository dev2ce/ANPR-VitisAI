...
def gather_data(dataset, image_format, labels_anchors_file) :
	...
	for data_file in files :
		...
		for anchor in anchors:
			...
			line_content = 'test_set_char/' + data_file.replace("txt", image_format)
			# Write data
...
def main():
	...
	parser.add_argument('-d',  '--dataset',      type=str,   default='test_set_char/eval-list',     help="Path to the dataset folder to read from. Default is './data/eval_set'.")
	parser.add_argument('-f',  '--image_format', type=str,   default='jpg',                 help="Format of the images to process. Default is 'jpg'.")
	parser.add_argument('-o',  '--output_file',  type=str,   default='test_char_eval_list.txt',  help="TensorBoard port number. Default is 'labels_anchors.txt'.")
