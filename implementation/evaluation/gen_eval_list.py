...
if not os.path.exists(folder_holding_yolo_files + os.sep + 'eval-list'):
...
for each_yolo_file in os.listdir(folder_holding_yolo_files):
  if each_yolo_file.endswith("txt"):
    ...
    if not image_name == each_yolo_file:
      ...
      # Start the XML file
      with open('eval-list' + os.sep + each_yolo_file.replace('txt', 'txt'), 'w') as f:
        f.write('1')
